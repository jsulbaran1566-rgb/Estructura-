...
from Excepciones.excepciones_reservas import ErrorStockInsuficiente
...
def obtener_reserva(
    ...
    if estado and estado not in estados_validos:
        return respuesta_error(f"Estado inválido. Permitidos: {estados_validos}", status_code=400)
    ...
    if not resultado:
        return respuesta_error("Reserva no encontrada con esos criterios", status_code=404)
...
def crear_reserva(
    ...
    if db.query(models.Reserva).filter(models.Reserva.id == id).first():
        return respuesta_error("Ya existe una reserva con ese id", status_code=400)

    lote = db.query(models.Lote).filter(models.Lote.producto.ilike(datos.producto)).first()
    if not lote:
        return respuesta_error("Producto no encontrado en los lotes disponibles", status_code=404)
